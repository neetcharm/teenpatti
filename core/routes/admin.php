...
Route::namespace('Auth')->group(function () {
    Route::middleware('admin.guest')->group(function () {
        Route::controller('LoginController')->group(function () {
            ...
            Route::post('/', 'login')->name('login')->middleware('throttle:8,1');
        ...
        Route::controller('ResetPasswordController')->group(function () {
            ...
            Route::post('password/reset/change', 'reset')->name('password.change')->middleware('throttle:5,1');
        });
    });
});
...
Route::middleware('admin')->group(function () {
    ...
    // Financial systems (Deposits/Withdrawals) removed — SaaS uses external WalletBridge.
    // Keep legacy route names registered so old views/controllers do not crash with RouteNotFoundException.
    $financialDisabledHandler = static function () {
        return to_route('admin.dashboard')->withNotify([
            ['warning', 'WalletBridge mode is enabled. Legacy financial module is disabled.'],
        ]);
    };

    $legacyFinancialRouteNames = [
    ...
    foreach ($legacyFinancialRouteNames as $legacyRouteName) {
        Route::any('financial-disabled/' . str_replace('.', '-', $legacyRouteName) . '/{legacy?}', $financialDisabledHandler)
            ->where('legacy', '.*')
    ...
    Route::controller('TenantController')
        ...
        ->group(function () {
            ...
            Route::post('regenerate/{id}',  'regenerateKeys')->name('regenerate');
            Route::get('sessions/{id}',     'sessions')->name('sessions');
            Route::get('transactions/{id}', 'transactions')->name('transactions');
            Route::get('games/{id}',        'games')->name('games');
            Route::post('games/{id}',       'updateGames')->name('games.update');
            Route::post('test-db/{id}',     'testDb')->name('test.db');
        });
