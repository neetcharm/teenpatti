...
trait SupportTicketManager
{
    ...
    public function ticketDownload($attachmentId)
    {
        ...
        if (!$attachment || !$attachment->supportMessage || !$attachment->supportMessage->ticket) {
            if ($this->apiRequest) {
                $notify[] = 'Attachment not found';
                return responseError('attachment_not_found', $notify);
            }
        ...
        if ($this->userType != 'admin') {
            ...
            $column = $this->column;
            if ((int) $attachment->supportMessage->ticket->$column !== (int) $this->user->id) {
                if ($this->apiRequest) {
                    $notify[] = 'Unauthorized user';
                    return responseError('unauthorized', $notify);
                }
        ...
        $path     = getFilePath('ticket');
        $fullPath = safeDownloadPath($path . '/' . $file, [$path]);
        if (!$fullPath) {
            if ($this->apiRequest) {
                $notify[] = 'Attachment not found';
                return responseError('attachment_not_found', $notify);
        ...
        $title = slug($attachment->supportMessage->ticket->subject ?: 'support-ticket');
