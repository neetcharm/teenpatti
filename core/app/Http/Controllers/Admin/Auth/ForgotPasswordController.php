...
class ForgotPasswordController extends Controller {
    ...
    public function verifyCode(Request $request) {
        ...
        $adminPasswordReset = AdminPasswordReset::where('email', $email)->where('status', Status::ENABLE)->orderBy('id', 'desc')->first();
        if (!$adminPasswordReset || !hash_equals((string) $adminPasswordReset->token, (string) $request->code)) {
            $notify[] = ['error', 'Verification code does not match'];
            return to_route('admin.login')->withNotify($notify);
        }

        if ($this->isCodeExpired($adminPasswordReset->created_at)) {
            AdminPasswordReset::where('email', $email)->update(['status' => Status::DISABLE]);
