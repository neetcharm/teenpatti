<?php
    ...
    exit('Installer is disabled after setup.');
    }
    ...
    if (version_compare(PHP_VERSION, (string) $requiredPHP, '>=')) {
    ...
    if ($action == 'result') {
    $response = ['error' => 'ok', 'location' => '../core/.env', 'body' => "APP_NAME=\"{{app_name}}\"\nAPP_ENV={{app_env}}\nAPP_KEY=base64:{{key}}\nAPP_DEBUG={{app_debug}}\nAPP_TIMEZONE=UTC\nAPP_URL={{url}}\n\nAPP_LOCALE=en\nAPP_FALLBACK_LOCALE=en\nAPP_FAKER_LOCALE=en_US\n\nAPP_MAINTENANCE_DRIVER=file\nAPP_MAINTENANCE_STORE=database\n\nBCRYPT_ROUNDS=12\n\nLOG_CHANNEL=stack\nLOG_STACK=single\nLOG_DEPRECATIONS_CHANNEL=null\nLOG_LEVEL={{log_level}}\n\nDB_CONNECTION=mysql\nDB_HOST={{db_host}}\nDB_PORT={{db_port}}\nDB_DATABASE={{db_name}}\nDB_USERNAME={{db_user}}\nDB_PASSWORD=\"{{db_pass}}\"\n\nSESSION_DRIVER=database\nSESSION_LIFETIME=120\nSESSION_ENCRYPT=false\nSESSION_PATH=/\nSESSION_DOMAIN=null\n\nBROADCAST_CONNECTION=log\nFILESYSTEM_DISK=local\nQUEUE_CONNECTION=database\n\nCACHE_STORE=database\nCACHE_PREFIX=\n\nPURCHASECODE=\"{{code}}\"\n"];

    if ($response['error'] == 'ok') {
        try {
            $dbPort = trim((string) ($_POST['db_port'] ?? (isLocalUrl($_POST['url'] ?? '') ? '3307' : '3306')));
            $db     = new PDO("mysql:host=$_POST[db_host];port=$dbPort;dbname={$_POST['db_name']}", $_POST['db_user'], $_POST['db_pass']);
            ...
            $response['error']   = 'error';
            $response['message'] = (isset($_POST['db_type']) && $_POST['db_type'] == 'create-new-database') ? 'There is a problem with creating the database.' : 'Database Credential is Not Valid. ' . $e->getMessage();
        }
    }
    ...
    if (isset($response['error']) && $response['error'] == 'ok') {
        try {
            ...
            $isLocalInstall = isLocalUrl($_POST['url']);
            $arr['key']     = base64_encode(random_bytes(32));
            $arr['app_name'] = $itemName;
            $arr['app_env'] = $isLocalInstall ? 'local' : 'production';
            $arr['app_debug'] = $isLocalInstall ? 'true' : 'false';
            $arr['log_level'] = $isLocalInstall ? 'debug' : 'warning';
            $arr['url']     = $_POST['url'];
            $arr['db_host'] = $_POST['db_host'];
            $arr['db_port'] = trim((string) ($_POST['db_port'] ?? ($isLocalInstall ? '3307' : '3306')));
    ...
    if (isset($response['error']) && $response['error'] == 'ok') {
        try {
            $stmt = $db->prepare("UPDATE admins SET email=?, username=?, password=? WHERE username='admin'");
            $stmt->execute([$_POST['email'], $_POST['admin_user'], password_hash($_POST['admin_pass'], PASSWORD_DEFAULT)]);
        } catch (Exception $e) {
...
<head>
	...
	<title>Easy Installer by Vaivitech</title>
	...
	<link rel="shortcut icon" href="https://license.vaivitech.com/external/favicon.png" type="image/x-icon">
</head>
...
<body>
	<header class="py-3 border-bottom border-primary bg--dark">
		<div class="container">
			<div class="d-flex align-items-center justify-content-between header gap-3">
				<img class="logo" src="https://license.vaivitech.com/external/logo.png" alt="Vaivitech">
							...
							<?php
                                if ($action == 'result') {
                                    ...
                                    if (isset($response['error']) && $response['error'] == 'ok') {
                                        ...
                                        echo '<div class="warning mt-2"><h5 class="mb-4 fw-normal">Try again. Or you can ask for support by creating a support ticket.</h5><a href="?action=information" class="theme-button choto me-1 mb-3">Try Again</a> <a href="https://vaivitech.com/support" target="_blank" class="theme-button choto ms-1">create  ticket</a></div>';
								...
								<form action="?action=result" method="post" class="information-form-area mb--20">
									...
									<div class="info-item">
										...
										<div class="row">
											...
											<div class="information-form-group col-sm-6">
												<input type="text" name="db_port" placeholder="Database Port" value="<?php echo isLocalUrl(appUrl()) ? '3307' : '3306'; ?>" required>
											</div>
											<div class="information-form-group col-sm-6">
	...
	<footer class="py-3 text-center bg--dark border-top border-primary">
		<div class="container">
			<p class="m-0 font-weight-bold">&copy;<?php echo Date('Y') ?> - All Right Reserved by <a href="https://vaivitech.com/">Vaivitech LLC</a></p>
		</div>
	</footer>
