...
function showRoundSummary(result) {
    ...
    $("#tpSummaryTitle").text("Round #" + roundNum + " - Complete Summary");
...
function renderCountdown() {
    $("#tpTimer").text(Math.max(0, Math.floor(countdownSeconds)));
}
...
function syncCountdown(serverRemaining) {
    ...
    var diff = Math.abs(remaining - countdownSeconds);
    if (!hasSyncedOnce || diff > 1) {
