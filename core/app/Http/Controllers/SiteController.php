...
class SiteController extends Controller {
    ...
    public function liveStats($alias = null) {
        ...
        $historyLimit = max(1, min((int) request('history_limit', request('limit', 20)), 60));
        $highestLimit = max(1, min((int) request('highest_limit', 12), 30));

        $logs = GameLog::with(['user:id,username,firstname,lastname', 'game:id,name,alias'])
            ->where('demo_play', Status::NO)
            ->whereHas('game', function ($query) use ($alias) {
                $query->active();
                if ($alias) {
        ...
        $stats = $logs->map(function ($log) {
            return [
                'id'             => $log->id,
                'user'           => $this->maskLiveUser($log->user),
                'game'           => @$log->game->name,
                'alias'          => @$log->game->alias,
                'invest'         => showAmount($log->invest, currencyFormat: false),
                'win_amount'     => showAmount($log->win_amo, currencyFormat: false),
                'status'         => $log->status == Status::GAME_FINISHED ? ($log->win_status == Status::WIN ? 'win' : 'loss') : 'running',
                'result'         => $this->formatLiveResult($log),
                'created_at'     => showDateTime($log->created_at, 'H:i:s'),
                'created_human'  => diffForHumans($log->created_at),
            ];
        })->values();

        $highestWinLogs = GameLog::with(['user:id,username,firstname,lastname', 'game:id,name,alias'])
            ->where('demo_play', Status::NO)
        ...
        $highestStats = $highestWinLogs->map(function ($log) {
            $invest = (float) $log->invest;
            $winAmo = (float) $log->win_amo;
            ...
            return [
                ...
                'invest'         => showAmount($log->invest, currencyFormat: false),
                'win_amount'     => showAmount($log->win_amo, currencyFormat: false),
                'multiplier'     => $invest > 0 ? getAmount($winAmo / $invest) : 0,
                'result'         => $this->formatLiveResult($log),
                'created_at'     => showDateTime($log->created_at, 'H:i:s'),
                'created_human'  => diffForHumans($log->created_at),
            ];
    ...
    private function maskLiveUser($user) {
        if (!$user) {
            return 'Player';
        }

        $username = trim((string) ($user->username ?: trim(($user->firstname ?? '') . ' ' . ($user->lastname ?? ''))));
        if ($username === '') {
            $username = 'Player';
        }
        ...
        return mb_substr($username, 0, 2) . str_repeat('*', max($length - 3, 1)) . mb_substr($username, -1);
    }

    private function formatLiveResult($gameLog) {
        if (!$gameLog) {
            return null;
        }

        $rawResult = (string) $gameLog->result;
        $decoded   = json_decode($rawResult, true);

        if (json_last_error() === JSON_ERROR_NONE && $decoded !== null) {
            if (is_array($decoded)) {
                return Str::limit(collect($decoded)->flatten()->implode(','), 20);
            }
            return Str::limit((string) $decoded, 20);
        }

        if (is_numeric($rawResult)) {
            return getAmount((float) $rawResult);
        }

        return Str::limit($rawResult, 20);
    }
}
