...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        return response()->json([
            ...
            'data'    => [
                ...
                'userImage'          => getImage(getFilePath('userProfile') . '/' . $user->image, getFileSize('userProfile')),
            ],
        ]);
    }
    ...
    public function transactions(Request $request)
    {
        ...
        $user                    = Auth::user();
        $widget['total_balance'] = $user->balance;
        $widget['total_invest']  = GameLog::teenPatti()->where('user_id', $user->id)->sum('invest');
        $widget['total_win']     = GameLog::teenPatti()->win()->where('user_id', $user->id)->sum('invest');
    ...
    {
        $logs     = GameLog::teenPatti()->where('user_id', auth()->id())->where('status', 1)->where('demo_play', Status::NO)->latest()->with('game')->get();
        $notify[] = 'Game Logs';
        return responseSuccess('game_logs', $notify, $logs);
    }

    public function demoGameLog()
    {
        $logs     = GameLog::teenPatti()->where('user_id', auth()->id())->where('status', 1)->where('demo_play', Status::YES)->latest()->with('game')->get();
        $notify[] = 'Game Logs';
        return responseSuccess('game_logs', $notify, $logs);
    }
    ...
    public function userInfo()
    {
        ...
        return responseSuccess('user_info', $notify, [
            'user' => auth()->user(),
            'userImage' => getImage(getFilePath('userProfile') . '/' . auth()->user()->image, getFileSize('userProfile')),
        ]);
    }
