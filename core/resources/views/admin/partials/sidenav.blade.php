            ...
            <ul class="sidebar__menu">
                @foreach ($sideBarLinks as $key => $data)
                            ...
                            <div class="sidebar-submenu {{ menuActive(@$data->menu_active, 2) }} ">
                                <ul>
                                    @foreach ($data->submenu as $menu)
                                        ...
                                        @endphp
                                        @php
                                            $submenuRouteName = (string) (@$menu->route_name ?? '');
                                            $submenuHref = \Illuminate\Support\Facades\Route::has($submenuRouteName)
                                                ? route($submenuRouteName, $submenuParams)
                                                : 'javascript:void(0)';
                                        @endphp
                    ...
                    @else
                        ...
                        @endphp
                        @php
                            $mainRouteName = (string) (@$data->route_name ?? '');
                            $mainHref = \Illuminate\Support\Facades\Route::has($mainRouteName)
                                ? route($mainRouteName, $mainParams)
                                : 'javascript:void(0)';
                        @endphp
                        <li class="sidebar-menu-item {{ menuActive(@$data->menu_active) }}">
                            <a href="{{ $mainHref }}" class="nav-link ">
                                <i class="menu-icon {{ $data->icon }}"></i>
                                <span class="menu-title">{{ __(@$data->title) }}</span>
