                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    @auth
                        <ul class="navbar-nav main-menu m-auto">
                            ...
                            @php
                                $hasDepositNowRoute = Route::has('user.deposit.index');
                                $hasDepositHistoryRoute = Route::has('user.deposit.history');
                                $hasWithdrawRoute = Route::has('user.withdraw');
                                $hasWithdrawHistoryRoute = Route::has('user.withdraw.history');
                            @endphp

                            @if ($hasDepositNowRoute || $hasDepositHistoryRoute)
                                <li class="menu_has_children">
                                    <a href="#">@lang('Deposit')</a>
                                    <ul class="sub-menu">
                                        @if ($hasDepositNowRoute)
                                            <li><a href="{{ route('user.deposit.index') }}">@lang('Deposit')</a></li>
                                        @endif
                                        @if ($hasDepositHistoryRoute)
                                            <li><a href="{{ route('user.deposit.history') }}">@lang('Deposit Log')</a></li>
                                        @endif
                                    </ul>
                                </li>
                            @endif
                            @if ($hasWithdrawRoute || $hasWithdrawHistoryRoute)
                                <li class="menu_has_children">
                            ...
                            <li class="menu_has_children">
                                ...
                                <ul class="sub-menu">
                                    @if (gs('balance_transfer') && Route::has('user.transfer.index'))
